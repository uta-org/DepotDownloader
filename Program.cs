    ...
    internal class Program
    {
        ...
        private static async Task<int> MainAsync(string[] args)
        {
            ...
            else
            {
                ...
                var manifestId = GetParameter(args, "-ugc", ContentDownloader.INVALID_MANIFEST_ID);
                ...
                {
                    depotId = GetParameter(args, "-depot", ContentDownloader.INVALID_DEPOT_ID);
                    manifestId = GetParameter(args, "-manifest", ContentDownloader.INVALID_MANIFEST_ID);
                    if (depotId == ContentDownloader.INVALID_DEPOT_ID &&
                        manifestId != ContentDownloader.INVALID_MANIFEST_ID)
                    {
                ...
                if (InitializeSteam(username, password))
                    try
                    {
                        await ContentDownloader
                            .DownloadAppAsync(appId, depotId, manifestId, branch, os, arch, language, lv, isUGC)
